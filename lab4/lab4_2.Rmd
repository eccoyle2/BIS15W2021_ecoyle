...
```{r}
mammals_narrow<-select(mammals,"family","mass","genus","species","gestation")
mammals_narrow
```

5. Use the output from #4 to focus on the family Bovidae with a mass greater than 450000.
```{r}
bovidae<-filter(mammals_narrow,family=="Bovidae",mass>450000)
bovidae
```
...
```{r}
primates1<-filter(mammals,genus=="Lophocebus"|genus=="Erythrocebus"|genus=="Macaca")
primates1
```

```{r}
primates2<-select(primates1,"mass","gestation","newborn")
primates2
```
```{r}
min(primates2$newborn)
```
```{r}
filter(primates1,newborn==389.5)
```
