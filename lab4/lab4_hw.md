...
## [1] 34137012
```
#On average, that data suggests that herbivores have a larger mean.hra.m2 or mean homerange area in m2.
...
```r
deer_specific<-select(deer,"mean.mass.g","log10.mass","family","genus","species")
deer_specific
```
...
```r
deer_specific[with(deer_specific,order(-log10.mass)),]
```
...
```r
homebodied_snake<-filter(snakes,species=="schneideri")
homebodied_snake
```
...
## #   PPMR <dbl>, prey.size.reference <chr>
```
#the namaqua dwarf adder is very small (up to 28 cm on average) and is found in Nambia where it also known as the Namakwadwergadder. It has cytotoxic venom that target the cells and tissue structure immediately around the bite resulting in pain and swelling. It cannot be counteracted by antivenom but is not usually fatal to humans.
