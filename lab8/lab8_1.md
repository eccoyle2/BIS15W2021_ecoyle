...
```r
relig_income<-readr::read_csv(file="data/relig_income.csv")
```
...
# column names are actually data- income
...
```r
plant_data<-readr::read_csv(file="data/plant_data.csv")
```
...
#the days are actually data (column names) and force the number of day to be a character
